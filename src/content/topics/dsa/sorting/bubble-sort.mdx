...
If the array is perfectly reverse sorted `[5, 4, 3, 2, 1]`, it requires $(N-1) + (N-2) + (N-3)...$ comparisons, which is the mathematical sum $\frac{N(N-1)}{2}$, dropping the $O(N^2)$ atomic bomb.
