...
In distributed NoSQL systems, scaling across hundreds of servers makes strict ACID transactions nearly impossible without destroying performance (see the [CAP Theorem](/topics/system-design/fundamentals/cap-theorem)). Instead, these systems embrace the **BASE** philosophy: Basically Available, Soft state, Eventual consistency.
...
```mermaid
flowchart LR
    User["User Clicks Like"] --> NodeA["Node A (US East)<br/>Likes: 101"]
    
    NodeA -.->|"Async Replication<br/>(Eventually...)"| NodeB["Node B (Europe)<br/>Likes: 101"]
    
    UserEU["European User"] --> NodeB
    Note1["If UserEU requests data<br/>before replication finishes,<br/>they see 100 Likes. That's fine!"]
```
